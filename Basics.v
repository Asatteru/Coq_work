...
Proof. unfold nandb. simpl. reflexivity. Qed.
Example test_nandb4 := eq_refl : (nandb true true) = false.
(** [] *)
...
Proof.
  intros [].
  - intros c. simpl. intros. rewrite -> H. reflexivity.
  - intros c. simpl. intros. assumption.
Qed.
(** [] *)
...
Fixpoint bin_to_nat (b : bin) : nat :=
  match b with
  | Z => O
  | T b' => mult (bin_to_nat b') (S (S O))
  | N b' => plus (mult (bin_to_nat b') (S (S O))) (S O)
  end.
(*
...
Compute (bin_to_nat (N (N (N Z)))).

                    
Example test_bin_incr1 : incr (N (N Z)) = T (T (N Z)).
Proof. simpl. reflexivity. Qed.

Example test_bin_incr2 : incr Z = N Z.
Proof. simpl. reflexivity. Qed.

Example test_bin_to_nat1 : bin_to_nat (N (N Z)) = S (S (S O)).
Proof. simpl. reflexivity. Qed.
       
Example test_bin_to_nat2 : bin_to_nat (T ((N (N Z)))) = S (S (S (S (S (S O))))).
Proof. simpl. reflexivity. Qed.

Example test_bin_to_nat3 : bin_to_nat Z = O.
Proof. simpl. reflexivity. Qed.   
(** [] **)
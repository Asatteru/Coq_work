...
(** For example, suppose we want to prove that [(n + m) + (p + q)
    ...
    commutativity of addition ([plus_com]) to rewrite one into the
 ...
 (*
   ...
   Theorem: [n + m = m + n] for any [n, m].
    ...
    - First, [n = 0].
     ...
     By the difinition of [+] and [plus_r_0], this follows from
      
       m = m.

     Which is immidiate from reflexivity.
    ...
    - Next, [n = S n'], where
     ...
     By the difinition of [+], [plus_n_Sm] this follows from

        n' + m = m + n'

     Which is immidiate from the I.H. 
...
(** Write an informal proof of the following theorem, using the
    ...
    - First, [n = 0].
     ...
     This follow directly from definition.
    ...
    - Next, [n = S n'], where
     ...
     By the difinition of [beq_nat], this follows from
...
Proof.
  ...
  (*rewrite -> plus_comm. assert (H: n + p = p + n).
  { rewrite -> plus_comm. reflexivity. }
...
Proof.
  ...
  - simpl. rewrite -> IHm'. rewrite -> mult_r. reflexivity.
Qed.
     (** [] *) 
...
Proof.
  ...
  - simpl. rewrite <- IHb'. rewrite <- plus_1_l. symmetry. rewrite -> mult_comm. rewrite -> mult_r. rewrite -> plus_swap.
    replace (1 + 1) with 2. rewrite -> plus_comm. rewrite -> mult_comm. reflexivity.
     reflexivity.
Qed.
(** [] *)
...
(*
Fixpoint is_2bai (n : nat) : bool :=
  match n with
  | O => true
  | S O => false
  | S n => is_2bai (minus (S n) (S (S O)))
  end.

Compute (is_2bai 100).

Fixpoint div_by2 (n : nat) : nat :=
  match n with
  | O => O
  | S O => O
  | S (S n') => 1 + div_by2 n'
...
Qed.
    ...
    (b) You might naturally think that we should also prove the
        ...
        b = T b' の時, b’が何であるかが分からないためinductionを進められない。
...
Compute (nat_to_bin (bin_to_nat  (N (N (N Z))) )).
...
(*
Theorem bin_to_nat_bin_pres : forall b : bin,
